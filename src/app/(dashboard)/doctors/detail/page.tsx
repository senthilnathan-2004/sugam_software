...
import { User, Calendar, FileText, HeartPulse, Clock } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { DataTable } from '@/components/common/data-table';
import { ColumnDef } from '@tanstack/react-table';

// Doctor id comes from the `?id=` query string (read client-side). Path params
// (`/doctors/detail/[id]`) can't work under `output: 'export'`.
function DoctorDetailContent() {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="truncate max-w-50 block">{row.getValue('chiefComplaint')}</span>,
    },
    {
